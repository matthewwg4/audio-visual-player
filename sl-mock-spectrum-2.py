...
class AudioVisualizer(Frame):
    ...
    def updateUI(self):
        ...
        if self.next_beat != len(self.beats) and song_time > self.beats[self.next_beat]:
            self.next_beat += 1
            self.colors += 0.17 #random.uniform(0.13, 0.18)
        else:
            self.colors += 0.005
        ...
        for i in range(ranges):
            ...
            startX = 20+i*20
            stopX = 35+i*20
            color = '#'+(''.join(('0'+hex(int(x*255))[2:])[-2:] for x in colorsys.hsv_to_rgb(self.colors,1,proportion)))
