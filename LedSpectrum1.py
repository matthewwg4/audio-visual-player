...
class LEDLights:
    ...
    def update(self):
        ...
        for i in range(self.ranges):
            ...
            decibels[i] -= 0.35 * (((i+1)/self.ranges)**3) * decibels[i]
        ...
        for i in range(ranges):
            ...
            color = colorsys.hsv_to_rgb(self.colors[i],1,proportion)
            red.append(color[0])
...
def main():
    ...
    freq_meta_groups = [
        ...
        {'start':60, 'stop':250, 'count':8},     # bass
        {'start':250, 'stop':500, 'count':7},    # low-midrange
        {'start':500, 'stop':2000, 'count':9},   # midrange
        {'start':2000, 'stop':4000, 'count':7},  # upper-midrange
        {'start':4000, 'stop':6000, 'count':6},  # presence
        {'start':6000, 'stop':11000, 'count':6}  # brilliance
    ]
    freq_groups = []
