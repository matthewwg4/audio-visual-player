...
class LEDLights:
    ...
    def __init__(self, code_directory, songs_file, freq_groups, smoothing=0, sample_per=0.04):
    ...
    def update(self):
        ...
        midpoint, power = self.analyzer.get_midpoint(self.last_time-self.start_time, song_time)
        ...
        pow_range = self.max-self.min
        if power < self.min + pow_range * 0.0002:
            self.min = power
        else:
            self.min += pow_range * 0.0002 # slight increase
        if power > self.max - pow_range * 0.0002:
            self.max = power
        else:
            self.max -= pow_range * 0.0002 # slight decrease
        proportion = (power-self.min)/(self.max-self.min)
    ...
    def nextSongName(self):
        ...
        with open(self.songs_file) as file:
            ...
            while next_line and (not found_song):
                pos_song_path = os.path.join(self.songs_folder, next_line.strip())
                if path.exists(pos_song_path):
        ...
        print("Next song: {}".format(song_name)[:-4])
...
def main(code_directory="."):
    ...
    lights = LEDLights(code_directory, "songs.txt", freq_groups, smoothing=0, sample_per=0.04)
    delay = lights.setup()
    time.sleep(delay)
    while True:
        delay = lights.update()
